...
const VerifyPhone: React.FC<ScreenProps> = ({ navigation }) => {
  ...
  const { setLoggedIn, setProfileStatus } = useProfileStatusStore();
  ...
  const handleSendOtp = () => {
    ...
    // Handle: Show verification screen on success, show error on failure
  ...
  const onCodeComplete = (otp: string) => {
    ...
    // Send: { phoneNumber: string, otp: string }
    // Expect: { success: boolean, message: string }
    // Handle: Set user as logged in, navigate to HomeStack on success, show error on failure
  ...
  return (
    <SAScrollView
      ...
      header={<Header title='Complete account setup' />}
    >
